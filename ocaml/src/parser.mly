...
pipe_test: /* Pipe operator: a, a |> b */
  | or_test { ([$1], dummy_pos) }
  | or_test PIPE pipe_test { ($1::$3, snd $2) }
...
expr_statement: /* Expression statement: a + 3 - 5 */
  ...
  | test_list EQ test_list 
    { Assign ($1, $3, false, snd $2) }
  | test_list ASSGN_EQ test_list 
    { Assign ($1, $3, true, snd $2) }
%inline aug_eq: 
  /* TODO: bit shift ops */
...
case_suite:
  ...
  | case; rest = case_suite 
    { $1::rest }
case:
  | CASE INT ELLIPSIS INT 
    {  }
  | CASE SEQ
  | CASE test COLON suite 
    { (Some $2, None, $4, $1) }
  | CASE test AS ID COLON suite 
    { (Some $2, Some (fst $4, snd $4), $6, $1) }
case_type:
  | INT ELLIPSIS INT { RangePattern($1, $3) }
  | INT ELLIPSIS INT { RangePattern($1, $3) }
import_statement:
