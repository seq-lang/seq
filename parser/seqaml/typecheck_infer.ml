...
let rec unify ?(on_unify=ignore2) (t1 : Ann.t) (t2 : Ann.t) =
  ...
  | { typ = Func { f_name = n1; f_generics = g1; f_args = a1; f_ret = r1; _ }; _ },
    ...
    when n1 = n2 ->
    let s1 = sum_or_neg g1 g2
      ~f:(fun (n1, (_, t1)) (n2, (_, t2)) -> if n1 <> n2 then -1 else u t1 t2)
    in
    if s1 = -1 then -1 else
    let s2 = sum_or_neg a1 a2
      ~f:(fun (n1, t1) (n2, t2) -> if n1 <> n2 then -1 else u t1 t2)
    in
    if s1 = -1 then -1 else
    let s3 = u r1 r2 in
  ...
  | t, ({ typ = TypeVar ({ contents = Unbound u' } as tv'); _ } as t') ->
    let occ = occurs u' t ~if_unify:(fun level -> function
      | { contents = Unbound (i', l', g') } as tv' ->
        if l' > level then tv' := Unbound (i', level, g')
      | _ -> ())
    in
    if not occ then begin match t with
      | { typ = TypeVar ({ contents = Unbound (i, _, _) } as tv); _ }
        when i > (fst3 u') ->
        on_unify t t' (* t := t' *)
        (* (upos, tv) t' *)
      | _ ->
...
let unify_inplace ~ctx t1 t2 =
  let u = unify t1 t2 ~on_unify:(fun lht rht ->
    let lhs_link = match lht.typ with
      | TypeVar ({ contents = Unbound _} as l) -> l
    ...
    | TypeVar { contents = Unbound (i, _, true) } ->
      ...
      Util.dbg "[WARN] attempting to restring generic T%d to %s" i (Ann.typ_to_string rht)
    | _ ->
      (* u := pos :: !u;  *)
      lhs_link := Bound rht)
  in
...
let rec generalize ~ctx ~level (t : Ann.t) =
  ...
  | Tuple args ->
    { t with typ = Tuple (List.map args ~f:g) }
  | TypeVar { contents = Bound t } ->
    generalize ~ctx ~level t
...
let rec generalize_inplace ~(ctx : C.t) (t : Ann.t) =
  ...
  match t.typ with
  | Class ({ c_generics; _ } as c) ->
    List.iter c_generics ~f:(fun (_, (_, t)) -> g t);
  | Func ({ f_generics; f_args; f_ret; _ } as fn) ->
...
let instantiate ~(ctx : C.t) ?(inst=Int.Table.create ()) ?parent (t : Ann.t) =
  ...
  let rec f (t : Ann.t) =
    ...
    | TypeVar { contents = Generic g } ->
      begin match Hashtbl.find inst g with
        | Some s -> s
        | None ->
    ...
    | Func fn ->
      { t with
        typ = Func { fn with
          ...
          f_ret = f fn.f_ret } }
    | Tuple args ->
      { t with typ= Tuple (List.map args ~f) }
    | _ -> t
  in
  let parent = match parent, (Ann.real_type t).typ with
    | Some _, _ -> parent
    | None, Class { c_parent = _, Some p; _ }
    | None, Func { f_parent = _, Some p; _ } -> Some p
    | _ -> None
  in
  parent
  >>| traverse_parents ~ctx ~f:(fun n (key, data) -> Hashtbl.set inst ~key ~data)
  |> ignore;
  f t
