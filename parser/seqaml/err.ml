...
let split_error msg =
  ...
  raise @@ SeqCError (msg, Ast_ann.{ file; line; col; len })
...
let to_string ?file ~pos_lst err =
  ...
  let lines =
    List.mapi pos_lst ~f:(fun i pos ->
        let Ast_ann.{ file; line; col; len } = pos in
