...
module Typecheck (E : Typecheck_intf.Expr) (R : Typecheck_intf.Real) : Typecheck_intf.Stmt = struct
  open Ast
  open Ast.Stmt
  ...
  let rec parse ~(ctx: C.t) (s : Stmt.t Ann.ann) : Stmt.t Ann.ann list =
    let ann, node = s in
    C.push_ann ~ctx ann;
    let (node : Stmt.t) = match node with
      ...
      | _ ->
        snd @@ Stmt.walk s ~fe:(E.parse ~ctx) ~f:(fun s -> match parse ~ctx s with
          | [s] -> s
          | _ -> C.err ~ctx "whoops, expected 1 statement")
    in
  ...
  and parse_realized ~(ctx: C.t) stmts =
    ...
    Stack.iter ctx.env.unbounds ~f:(fun u ->
      if Ann.has_unbound u then C.err "unbound %s is not realized" (Ann.to_string u));
    stmts
  ...
  and parse_assign ctx ?(prefix="") (lhs, rhs, shadow, dectyp) =
    let rhs = E.parse ~ctx:(C.enter_level ~ctx) rhs in (** TODO: optional rhs / shadow *)
    match snd lhs, dectyp with
    | Id var, _ ->
      let typ = fst rhs in
      dectyp >>| E.parse ~ctx >>| fst >>| T.unify_inplace ~ctx typ |> ignore;
      ( match Ctx.in_scope ~ctx var with
        | Some (Var t | Type t) when T.unify t typ <> -1 ->
          T.unify_inplace ~ctx t typ
        | Some (Var t | Type t) ->
          Util.dbg "shadowing %s : %s with %s" var
            (Ann.typ_to_string t) (Ann.typ_to_string (fst rhs));
          Ctx.add ~ctx (prefix ^ var) (if typ.is_type_ast then Type typ else Var typ)
        | None ->
          Ctx.add ~ctx (prefix ^ var) (if typ.is_type_ast then Type typ else Var typ)
        | _ -> ierr ~ctx "[parse_assign] invalid assigment" );
      let typ = typ.typ in
      let f = fst lhs in
      let ann = { f with typ } in
    ...
    | Index (var_expr, [index_expr]), None -> (* a[x] = b *)
      Expr (E.parse ~ctx @@ C.eannotate ~ctx @@
        e_call (e_dot var_expr "__setitem__") [ index_expr; rhs ])
  ...
  and parse_del ctx expr =
    ...
    | Expr.Index (lhs, [index]) ->
      Expr (E.parse ~ctx @@ C.eannotate ~ctx @@
        e_call (e_dot lhs "__delitem__") [ index ])
    | Id var ->
      Ctx.remove ~ctx var;
  ...
  and parse_print ctx (exprs, term) =
    let exprs = List.map exprs ~f:(fun e ->
        E.parse ~ctx @@ C.eannotate ~ctx @@ e_call (e_dot e "__str__") [])
    in
    Print (exprs, term)

  and parse_return ctx expr =
    let t, expr = match expr with
  ...
  and parse_while ctx (cond, stmts) =
    let cond = E.parse ~ctx @@ C.eannotate ~ctx @@ e_call (e_dot cond "__bool__") [] in
    let stmts = parse_block ctx stmts in
    While (cond, stmts)

  and parse_for ctx (for_vars, gen_expr, stmts) =
    let gen_expr = E.parse ~ctx @@ C.eannotate ~ctx @@ e_call (e_dot gen_expr "__iter__") [] in
    Ctx.add_block ~ctx;
    (* make sure it is ... ah, generator! *)
    let tvar = match (Ann.real_type (fst gen_expr)).typ with
      | Class { c_cache = ("generator", _); c_generics = [_, (_, t)]; _ } -> t
      | TypeVar { contents = Unbound _ } -> C.make_unbound ctx
      | _ -> C.err ~ctx "for expression must be an iterable generator %s" @@
             Ann.typ_to_string (fst gen_expr)
    in
    let for_var, init_stmts = match for_vars with
      | [for_var] ->
      ...
      | for_vars ->
        ...
        Ctx.add ~ctx for_var (Var tvar);
        for_var, List.mapi for_vars ~f:(fun idx v ->
          C.sannotate ~ctx @@ s_assign (e_id v) (e_index (e_id for_var) [e_int idx]));
    in
  ...
  and parse_if ctx cases =
    If (List.map cases ~f:(fun { cond; cond_stmts } ->
      let cond = cond >>| fun cond ->
        E.parse ~ctx @@ C.eannotate ~ctx @@ e_call (e_dot cond "__bool__") []
      in
  ...
  and parse_match ctx (what, cases) =
    ...
    Match (what, List.map cases ~f:(fun { pattern; case_stmts } ->
      Ctx.add_block ~ctx;
      let pat_t, pattern = match pattern with
  ...
  and parse_pattern ctx what_typ = function
    | StarPattern ->
      C.make_unbound ctx, StarPattern
    | BoundPattern _ ->
      C.err ~ctx "invalid bound pattern"
    | IntPattern i ->
      R.internal ~ctx "int", IntPattern i
    | BoolPattern b ->
      R.internal ~ctx "bool", BoolPattern b
    | StrPattern s ->
      R.internal ~ctx "str", StrPattern s
    | SeqPattern s ->
      R.internal ~ctx "seq", StrPattern s
    | RangePattern r ->
      R.internal ~ctx "int", RangePattern r
    | OrPattern tl ->
  ...
  and parse_import (ctx : C.t) ?(ext=".seq") imports =
    let import (ctx: C.t) file Stmt.{ from; what; import_as } =
      let vtable = match Hashtbl.find ctx.globals.imports file with
        | Some t -> t
        | None ->
          ...
          ictx.map
      in
      let additions = match what with
        | None -> (* import <from> (as <import as>) *)
          let name = Option.value import_as ~default:from in
          [name, C.Import file]
        | Some ["*", None] -> (* from <from> import <what := *> *)
          List.filter_map (Hashtbl.to_alist vtable)
            ~f:(function (s, []) -> None | (s, (h :: _)) -> Some (s, h))
        | Some lst -> (* from <from> import <what> *)
  ...
  and parse_try ctx (stmts, catches, finally) =
    let stmts = parse_block ctx stmts in
    let catches = List.map catches ~f:(fun { exc; var; stmts } ->
      Ctx.add_block ~ctx;
      let exc = match exc, var with
        | Some exc, Some var ->
          let exc = E.parse ~ctx exc in
          Ctx.add ~ctx var (Var (fst exc));
          Some exc
        | Some exc, None ->
          Some (E.parse ~ctx exc)
  ...
  and parse_extern ctx ?(prefix="") f =
    ...
    let args = List.map args ~f:(fun arg ->
      ...
      | None ->
        C.err ~ctx "extern functions must explicitly state parameter types")
    in
    let fret = match fn.typ with
    ...
    Ctx.add ~ctx fn.name (Var typ);
    let fn = { fn with typ = Some fret } in
    let node = C.ann ~typ ctx, Extern (lang, dylib, ctxn, fn, List.map ~f:fst args) in
    Hashtbl.set ctx.globals.realizations ~key:tf'.f_cache ~data:(node, String.Table.create ());
  ...
  and parse_function ctx ?(prefix="") ?(shalow=false) f =
    ...
    Ctx.add_block ~ctx;
    let explicits = List.map f.fn_generics ~f:(fun gen ->
    ...
    let fret = Option.value_map f.fn_name.typ
      ~default:(C.make_unbound ctx) ~f:(fun t -> fst (E.parse ~ctx t))
    in
    let fn_name = { name = fname; typ = None } in
    let fn_args, args = List.unzip @@ List.mapi f.fn_args ~f:(fun i arg ->
      let arg_typ = Option.map arg.typ ~f:(fun t -> E.parse ~ctx t) in
      let typ = match arg_typ with
    ...
    let tf' = Ann.{
      f_generics = List.map explicits ~f:(fun (n, (id, t)) ->
        n, (id, T.generalize ~ctx ~level:(ctx.env.level-1) t));
      f_name = fname;
      f_parent = ctx.env.enclosing_name, None;
      f_args = List.map args ~f:(fun (n, t) ->
    ...
    let fn_stmts = if shalow then f.fn_stmts else (
      let ctx = { ctx with
        env = { ctx.env with realizing = false
      ...
      let ret' = match !(ctx.env.C.enclosing_return) with
    ...
    let tfun = { (C.ann ctx) with typ = Func
      { tf' with f_generics = explicits ; f_args = args ; f_ret = fret } }
    in
  ...
  and parse_extend (ctx : C.t) (name, new_members) =
    let class_name = match snd name with
      | Expr.Id name -> name
    ...
    Ctx.add_block ~ctx;
    let ctx = C.enter_level ~ctx in
    let explicits = List.map tcls.c_generics ~f:(fun (gen, (id, t)) ->
  ...
  and parse_class ctx ?(prefix="") ?(is_type=false) cls =
    ...
    let explicits = List.map generics ~f:(fun gen ->
    ...
    let ctyp =
      if not is_type then None
      else Some (List.filter_map members ~f:(function
        | _, Assign (((_, Id name), _, _, typ) as d) -> typ >>| E.parse ~ctx >>| fst
        | _ -> None))
    in
    let members = String.Table.create () in
    let tc' = Ann.{
      (* seal for later use; dependents will still use same-numbered unbound
      while dependend recursive realization will reinstantiate it as another
      number *)
      c_generics = List.map explicits ~f:(fun (n, (id, t)) ->
        n, (id, T.generalize ~ctx ~level:(ctx.env.level - 1) t));
      c_name = class_name;
    ...
    in
    let tcls = C.make_link ~ctx { (C.ann ctx) with typ = Class tc' } in
    let ctx = { ctx with
      env = { ctx.env with
    ...
    in
    let cls_members = List.map (init :: cls.members) ~f:(fun s ->
      let prefix = sprintf "%s." class_name in
      let typ, node = match snd s with
        | Function f ->
        ...
        | Class c ->
          ...
          Hashtbl.set members ~key:c.class_name ~data:[Option.value_exn typ];
          typ, stmt
        | Assign (((_, Id name), _, _, _) as d) ->
          let stmt = parse_assign ctx ~prefix d in
          let typ = Ctx.in_block ~ctx (prefix ^ name) in
          let typ = match typ with Some (Var t | Member t | Type t) -> t | _ -> failwith "err" in
    ...
    List.iter2_exn cls.generics old_explicits ~f:(fun g t ->
      match t with Some t -> Ctx.add ~ctx g t | None -> ());
    List.iter explicits ~f:(fun (_, (_, t)) -> T.generalize_inplace ~ctx t);
  ...
  and parse_special ctx (name, exprs, _) =
    ierr "TODO"
    (* match name, exprs with
    | "%t_", [_, StmtNode.Expr e] ->
      let expr, _ = E.parse ctx e in
      db ">> %s |- %s" (ExprNode.to_string expr) (Ann.typ_to_string_full ~deep:false @@ TU.type_of expr);
      StmtNode.Pass ()
    | _ -> terr ~pos:ann.pos "not supported special" *)
end