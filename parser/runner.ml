...
let exec_string ctx ?(file = "<internal>") ?(debug = false) ?(jit = false) code
    =
  ignore
  @@ Codegen.parse ~file code ~f:(Codegen.Stmt.parse_module ~jit ~ctx)
...
let init file error_handler =
  ...
  try
    let ctx =
      Ctx.init_module ~filename:file ~mdl ~base:mdl
        ~block:(Llvm.Module.block mdl)
        (exec_string ~debug:false ~jit:false)
    in
    (* parse the file *)
    Ctx.parse_file ~ctx file ; Some ctx.mdl
  with Err.CompilerError (typ, pos) ->
    (* Ctx.dump ctx; *)
    error_handler typ pos ; None
...
let parse_c fname =
  let error_handler typ (pos : Ast.Ann.t list) =
    ...
    let msg =
      ...
      | Err.Parser ->
          "parsing error"
      | Lexer s | Descent s | Compiler s | Internal s ->
          s
    in
    Ctypes.(
      Foreign.foreign "caml_error_callback"
