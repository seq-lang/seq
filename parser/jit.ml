...
type t =
  ...
  ; tctx : Typecheck_ctx.t (** Execution context. *)
  ; lctx : Codegen_ctx.t (** Execution context. *)
  }
...
let init () : t =
  try
    ...
    let mdl = Llvm.JIT.init () in
    let block = Llvm.Block.func anon_fn in
    let lctx = Codegen_ctx.init_module ~filename ~mdl ~base:anon_fn ~block (Codegen_stmt.parse ~jit:true) in

    let jit = { cnt = 1; tctx; lctx } in
    Llvm.JIT.func lctx.env.mdl anon_fn;
    jit
  with
...
let c_init () =
  let hnd = init () in
  let p = Ctypes.raw_address_of_ptr hnd.lctx.env.mdl in
