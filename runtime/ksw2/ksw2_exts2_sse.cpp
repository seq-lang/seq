...
void ksw_exts2_sse41(void *km, int qlen, const uint8_t *query, int tlen,
                     ...
                     int8_t q, int8_t e, int8_t q2, int8_t noncan, int zdrop,
...
#define __dp_code_block2                                                       \
  ...
  // XXX: modified for Seq AGCT encoding
  if (flag & (KSW_EZ_SPLICE_FOR | KSW_EZ_SPLICE_REV)) {
    ...
    for (t = 0; t < tlen - 4; ++t) {
      ...
      if ((flag & KSW_EZ_SPLICE_FOR) && target[t + 1] == 1 &&
          target[t + 2] == 3)
        can_type = 1; // GTr...
      if ((flag & KSW_EZ_SPLICE_REV) && target[t + 1] == 2 &&
          target[t + 2] == 3)
        can_type = 1; // CTr...
      if (can_type && (target[t + 3] == 0 || target[t + 3] == 1))
        can_type = 2;
      if (can_type)
    ...
    for (t = 2; t < tlen; ++t) {
      int can_type = 0;
      if ((flag & KSW_EZ_SPLICE_FOR) && target[t - 1] == 0 && target[t] == 1)
        can_type = 1; // ...yAG
      if ((flag & KSW_EZ_SPLICE_REV) && target[t - 1] == 0 && target[t] == 2)
        can_type = 1; // ...yAC
      if (can_type && (target[t - 2] == 2 || target[t - 2] == 3))
        can_type = 2;
      if (can_type)
  ...
  for (r = 0, last_st = last_en = -1; r < qlen + tlen - 1; ++r) {
    ...
    if (st > 0) {
      ...
      x1 = -q - e, x21 = -q2;
      v1 = r == 0 ? -q - e
                  : r < long_thres ? -e : r == long_thres ? long_diff : 0;
    }
    if (en >= r) {
      ((int8_t *)y)[r] = -q - e;
      u8[r] = r == 0 ? -q - e
...
#else
        tmp2 =
            _mm_or_si128(_mm_andnot_si128(tmp, tmp2), _mm_and_si128(tmp, a2));
#endif
...
#else
        tmp2 =
            _mm_or_si128(_mm_andnot_si128(tmp, a2), _mm_and_si128(tmp, tmp2));
#endif
        _mm_store_si128(&x2[t], _mm_sub_epi8(tmp2, q2_));
        d = _mm_or_si128(
            d, _mm_andnot_si128(tmp,
