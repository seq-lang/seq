...
#define __dp_code_block2 \
	...
	// set the donor and acceptor arrays. TODO: this assumes 0/1/2/3 encoding!  XXX: modified for Seq AGCT encoding
	if (flag & (KSW_EZ_SPLICE_FOR|KSW_EZ_SPLICE_REV)) {
		...
		for (t = 0; t < tlen - 4; ++t) {
			...
			if ((flag & KSW_EZ_SPLICE_FOR) && target[t+1] == 1 && target[t+2] == 3) can_type = 1; // GTr...
			if ((flag & KSW_EZ_SPLICE_REV) && target[t+1] == 2 && target[t+2] == 3) can_type = 1; // CTr...
			if (can_type && (target[t+3] == 0 || target[t+3] == 1)) can_type = 2;
			if (can_type) ((int8_t*)donor)[t] = can_type == 2? 0 : semi_cost;
		}
		memset(acceptor, -noncan, tlen_ * 16);
		for (t = 2; t < tlen; ++t) {
			int can_type = 0;
			if ((flag & KSW_EZ_SPLICE_FOR) && target[t-1] == 0 && target[t] == 1) can_type = 1; // ...yAG
			if ((flag & KSW_EZ_SPLICE_REV) && target[t-1] == 0 && target[t] == 2) can_type = 1; // ...yAC
			if (can_type && (target[t-2] == 2 || target[t-2] == 3)) can_type = 2;
