...
/// Instr that represents a pipeline. Pipelines with only function
...
class PipelineInstr : public AcceptorExtend<PipelineInstr, Instr> {
public:
...
public:
  ...
  /// Constructs a pipeline instruction.
  /// @param stages vector of pipeline stages
  /// @param name the name
  explicit PipelineInstr(std::vector<Stage> stages = {}, std::string name = "")
      : AcceptorExtend(std::move(name)), stages(std::move(stages)) {}

  /// @return the pipeline type, or null if it contains generator stages
  const types::Type *getType() const override;
