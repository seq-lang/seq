...
void TryCatch::codegen0(BasicBlock*& block)
{
	...
	// finally:
	// logic here is complex since we potentially need to delegate to parent finally based on `depth`
	...
	if (this != root) {
		...
		builder.CreateCondBr(delegateNew, parents.front()->finallyStart, parents.front()->exceptionRouteBlock);
		...
		for (unsigned i = 0; i < tc->catchTypes.size(); i++) {
			if (!anyMatch(tc->catchTypes[i], catchTypesFull)) {
				...
				if (!tc->catchTypes[i] && !catchAll) {
					...
					builder.CreateStore(ConstantInt::get(seqIntLLVM(context), depth), delegateDepth);
	...
	if (catchAll)
		builder.CreateStore(ConstantInt::get(seqIntLLVM(context), catchAllDepth), delegateDepth);
	builder.CreateBr(catchAll ? (catchAllDepth > 0 ? finallyStart : catchAll) : finallyStart);

	builder.SetInsertPoint(exceptionRouteBlock);
	SwitchInst *switchToCatchBlock = builder.CreateSwitch(objType,
	                                                      defaultRouteBlock,
	                                                      (unsigned)handlersFull.size());
	for (unsigned i = 0; i < handlersFull.size(); i++) {
		...
		builder.CreateStore(ConstantInt::get(seqIntLLVM(context), depths[i]), delegateDepth);
		builder.CreateBr((i < handlers.size()) ? catchBlock : finallyStart);
		catchBlock = depthSet;

		if (catchTypesFull[i]) {
			switchToCatchBlock->addCase(
			  ConstantInt::get(IntegerType::getInt32Ty(context), (uint64_t)catchTypesFull[i]->getID(), true),
			  catchBlock);
		}
...
void Prefetch::codegen0(BasicBlock*& block)
{
	...
	for (unsigned i = 0; i < keys.size(); i++) {
		...
		idxType->callMagic("__prefetch__", {keyType}, idxVal, {keyVal}, block, getTryCatch());
	}
