...
let parse file code =
  ...
  try
    ...
    Stack.push 0 stack;
    let state = Lexer.{ stack = stack; offset = 0; ignore_newline = 0; fname = file } in
    let ast = Grammar.program (Lexer.token state) lexbuf in
    ast
  with
  | Grammar.Error ->
    let line = lexbuf.lex_start_p.pos_lnum + Lexer.global_offset.line in
    let col = lexbuf.lex_start_p.pos_cnum - lexbuf.lex_start_p.pos_bol + Lexer.global_offset.col in
    Printf.eprintf "[ocaml] parser error: %s:%d:%d\n%!" file line col;
    exit 1
  | Ast.GrammarError (s, pos)
  | Ast.SyntaxError (s, pos) ->
