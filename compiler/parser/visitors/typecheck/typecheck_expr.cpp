...
void TypecheckVisitor::visit(IdExpr *expr) {
  ...
  if (val->type->isStaticType()) {
    expr->isStaticExpr = true;
    assert(!expr->staticEvaluation.first);
    auto s = val->type->getStatic();
    if (s && s->staticEvaluation.first)
      resultExpr = transform(N<IntExpr>(s->staticEvaluation.second));
    return;
  } else if (val->isType()) {
    expr->markType();
  }
...
void TypecheckVisitor::visit(IfExpr *expr) {
  ...
  if (expr->cond->isStaticExpr) {
    if (expr->cond->staticEvaluation.first) {
      resultExpr =
          transform(expr->cond->staticEvaluation.second ? expr->ifexpr : expr->elsexpr,
                    false, allowVoidExpr);
...
void TypecheckVisitor::visit(BinaryExpr *expr) {
  static unordered_set<string> staticOps{"<",  "<=", ">", ">=", "==", "!=", "&&",
                                         "||", "+",  "-", "*",  "//", "%"};
  ...
  if (expr->lexpr->isStaticExpr && expr->rexpr->isStaticExpr &&
      in(staticOps, expr->op)) {
...
void TypecheckVisitor::visit(InstantiateExpr *expr) {
  ...
  for (int i = 0; i < expr->typeParams.size(); i++) {
    auto e = clone(expr->typeParams[i]); // need original expr w/o types
    expr->typeParams[i] = transform(expr->typeParams[i], true);
    TypePtr t = nullptr;
    if (expr->typeParams[i]->isStaticExpr) {
      t = make_shared<StaticType>(e, ctx);
    } else {
...
void TypecheckVisitor::visit(IndexExpr *expr) {
  ...
  if (expr->expr->isType() || typ->getFunc() || typ->getPartial()) {
    ...
    if (auto et = const_cast<TupleExpr *>(expr->index->getTuple()))
      ...
      it = {expr->index};
    resultExpr = transform(N<InstantiateExpr>(expr->expr, it));
    unify(expr->type, resultExpr->getType());
...
ExprPtr TypecheckVisitor::transformBinary(BinaryExpr *expr, bool isAtomic,
  ...
  if (expr->op == "&&") {
    return transform(N<IfExpr>(N<CallExpr>(N<DotExpr>(expr->lexpr, "__bool__")),
    ...
    return transform(N<CallExpr>(
        N<DotExpr>(N<BinaryExpr>(expr->lexpr, "is", expr->rexpr), "__invert__")));
  }
  ...
  return transform(N<CallExpr>(N<IdExpr>(method->funcName), expr->lexpr, expr->rexpr));
}
...
ExprPtr TypecheckVisitor::transformDot(DotExpr *expr, vector<CallExpr::Arg> *args) {
  ...
  if (expr->expr->isType()) {
    ...
    if (ctx->cache->functions[bestMethod->funcName].ast->attributes.has(Attr::Property))
      methodArgs.pop_back();
    ExprPtr e = N<CallExpr>(N<IdExpr>(bestMethod->funcName), methodArgs);
    ...
    if (mightChange) {
      if (!argClass) {
        ...
        if (argClass && !argClass->getFunc() && !argClass->getPartial())
          args[si].value = transform(N<DotExpr>(args[si].value, "__call__"));
...
string TypecheckVisitor::generateTupleStub(int len, const string &name,
  ...
  if (!ctx->find(typeName)) {
    ...
    StmtPtr stmt =
        make_shared<ClassStmt>(typeName, generics, args, nullptr, Attr({Attr::Tuple}));
...
string TypecheckVisitor::generateFunctionStub(int n) {
  ...
  if (!ctx->find(typeName)) {
    ...
    fns.emplace_back(
        make_shared<FunctionStmt>("__new__", clone(type), vector<Param>{}, params,
    ...
    fns.emplace_back(N<FunctionStmt>("__str__", N<IdExpr>("str"), vector<Param>{},
                                     params, N<SuiteStmt>(stmts)));
    params.clear();
    stmts.clear();
    // class Function.N[TR, T1, ..., TN]
    StmtPtr stmt = make_shared<ClassStmt>(typeName, generics, args, N<SuiteStmt>(fns),
...
void TypecheckVisitor::generateFnCall(int n) {
  ...
  if (!in(ctx->cache->classes[typeName].methods, "__call__")) {
    ...
    StmtPtr stmt = N<ClassStmt>(typeName, generics, vector<Param>{}, N<SuiteStmt>(fns),
                                Attr({Attr::Extend}));
    stmt = SimplifyVisitor::apply(ctx->cache->imports[STDLIB_IMPORT].ctx, stmt,
                                  FILE_GENERATED, 0);
    stmt = TypecheckVisitor(ctx).transform(stmt);
