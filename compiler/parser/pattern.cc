...
string StarPattern::to_string() const {
    return "#star";
}

IntPattern::IntPattern(int v) : value (v) {}
string IntPattern::to_string() const {
    return format("(#int {})", value);
}

BoolPattern::BoolPattern(bool v) : value (v) {}
string BoolPattern::to_string() const {
...
GuardedPattern::GuardedPattern(PatternPtr p, ExprPtr c) : pattern(move(p)), cond(move(c)) {}
